...
/* 3 functions to save and restore statuses + saveRandStatus      */
/* 1 function mtRand optimized for speed (MT is already one of    */
...
/* ------------------------------------------------------------- */
/* saveRandStatus     Saves a MT Random status in a text File    */
/*                                                               */
/* Input: inFileName  The file name                      D. Hill */
/* ------------------------------------------------------------- */

void saveRandStatus(char * inFileName)
{
    FILE *       fp  = fopen(inFileName, "w");
    int          i   = 0;
    unsigned int mtIndex;

    mtIndex = (unsigned int) genrand_int32() % 624;
    fprintf(fp, "%d\n", mtIndex);

    for(i = 0 ; i < N ; i++)
    {
       fprintf(fp, "%ld ", genrand_int32() );
    }
...
int main(int argc, char ** argv)
{
    ...
    for( k = 0; k < 4096 ; k++)
    {
       sprintf(fName,"mtsRand000M%06d",k);
       printf("%s\n", fName);
       saveRandStatus(fName);
    }
